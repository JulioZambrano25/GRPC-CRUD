...
# CRÍTICO: configurar encoding ANTES de importar psycopg2 y similares que dependan
# de la codificación del cliente. Ajustamos variables de entorno en Windows.
...
class UserService(hello_pb2_grpc.UserServiceServicer):
	...
	def CreateUser(self, request, context):
		...
		try:
			...
			return hello_pb2.UserReply(
				id=new_id, name=request.name, email=request.email, message="Usuario creado"
			)
	...
	def GetUser(self, request, context):
		...
		try:
			...
			if getattr(request, 'id', 0) == 0:
				...
				cur.close()
				if row:
					return hello_pb2.UserReply(id=row[0], name=row[1], email=row[2], message="Usuario encontrado")
				else:
	...
	def UpdateUser(self, request, context):
		...
		try:
			...
			if row:
				return hello_pb2.UserReply(
					id=row[0], name=request.name, email=request.email, message="Usuario actualizado"
				)
			else:
				return hello_pb2.UserReply(message="Usuario no encontrado")
		except Exception as e:
